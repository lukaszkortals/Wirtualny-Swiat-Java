...
public class Owca extends Zwierze{
    ...
    public void rozmnazanie() {
        ...
        if (this.getSwiat().getOrganizm(nowyX, nowyY)==null){
            this.getSwiat().setOrganizm(nowyX, nowyY, new Owca());
            this.getSwiat().getOrganizm(nowyX, nowyY).setSwiat(this.getSwiat());
            this.getSwiat().dodajZdarzenie("W [" + this.getPolozenieX() + "," + this.getPolozenieY() + "] pojawia się Owca!");
        }
    }
        ...
        for(int i=0 ; i< ilosc ; i++){    
            ...
            for (int j=0; j < max && !dodany ; j++)  {
                ...
                if (swiat.getOrganizm(x, y) == null){
                    swiat.setOrganizm(x, y, new Owca());
